...
(async function loadVolunteersTable() {
  ...
  function renderTable() {
    ...
    if (filtered.length === 0) {
      tbody.innerHTML = '<tr><td colspan="10" style="text-align:center;padding:2rem;color:var(--muted);font-size:0.9rem">لا توجد نتائج تطابق البحث</td></tr>';
      return;
    }
    ...
    for (var i = 0; i < filtered.length; i++) {
      ...
      var checked = selectedIndices[realIdx] ? ' checked' : '';
      var cbHtml = '<td style="text-align:center"><input type="checkbox" class="vol-bulk-cb" data-idx="' + realIdx + '"' + checked + '></td>';
      tr.innerHTML = cbHtml + '<td>' + vidDisplay + '</td><td>' + escapeHtml(v.fullName) + '</td><td>' + escapeHtml(v.email) + '</td><td dir="ltr">' + escapeHtml(v.phone) + '</td><td>' + escapeHtml(v.municipality) + '</td><td><span class="membership-text">' + memText + '</span></td><td>' + badge + '</td><td>' + acctStatus + '</td><td>' + actions + '</td>';
